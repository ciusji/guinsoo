...
import org.guinsoo.engine.UserBuilder;
 ...
 * {@link CredentialsValidator} configured for that realm.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Rights assignment can be managed through {@link UserToRolesMapper}
 * </p>
 * <p>
 * Default configuration has a realm Guinsoo that validate credentials through JAAS
 * api (appName=guinsoo). To customize configuration set guinsoo.authConfigFile system
 * property to refer a valid guinsooauth.xml config file
 * </p>
 */
public class DefaultAuthenticator implements Authenticator {
     ...
     * initialization is
     * <ol>
     * <li>Check guinsoo.authConfigFile system property.</li>
    ...
    public void init(Database database) throws AuthConfigException {
        ...
        synchronized (this) {
            ...
            URL guinsooAuthenticatorConfigurationUrl = null;
            try {
                ...
                if (configFile != null) {
                    if (trace.isDebugEnabled()) {
                        trace.debug("DefaultAuthenticator.config: configuration read from system property"
                                + " guinsooauth.configurationfile={0}", configFile);
                    }
                    guinsooAuthenticatorConfigurationUrl = new URL(configFile);
                }
                if (guinsooAuthenticatorConfigurationUrl == null) {
                    if (trace.isDebugEnabled()) {
                        trace.debug("DefaultAuthenticator.config: default configuration");
                    }
                    defaultConfiguration();
                } else {
                    configureFromUrl(guinsooAuthenticatorConfigurationUrl);
                }
            } catch (Exception e) {
                trace.error(e, "DefaultAuthenticator.config: an error occurred during configuration from {0} ",
                        guinsooAuthenticatorConfigurationUrl);
                throw new AuthConfigException(
                        "Failed to configure authentication from " + guinsooAuthenticatorConfigurationUrl, e);
            }
            initialized = true;
        }
    }
