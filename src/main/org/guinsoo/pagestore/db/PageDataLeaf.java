...
public class PageDataLeaf extends PageData {
    ...
    int addRowTry(Row row) {
        ...
        offsets = Page.insert(offsets, entryCount, x, offset);
        Page.add(offsets, x + 1, entryCount + 1, -rowLength);
        keys = Page.insert(keys, entryCount, x, row.getKey());
        rows = Page.insert(rows, entryCount, x, row);
        entryCount++;
    ...
    private void removeRow(int i) {
        ...
        start -= keyOffsetPairLen;
        offsets = Page.remove(offsets, entryCount + 1, i);
        Page.add(offsets, i, entryCount, rowLength);
        keys = Page.remove(keys, entryCount + 1, i);
        rows = Page.remove(rows, entryCount + 1, i);
    }
