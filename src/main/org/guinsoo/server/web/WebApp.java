...
import org.guinsoo.bnf.context.DbColumn;
import org.guinsoo.bnf.context.DbContents;
import org.guinsoo.bnf.context.DbSchema;
import org.guinsoo.bnf.context.DbTableOrView;
...
public class WebApp {
    ...
    private String tables() {
        ...
        try {
            ...
            session.put("tree", "");
            session.put("error", getStackTrace(0, e, isGuinsoo));
        }
        return "tables.jsp";
    }

    private String getStackTrace(int id, Throwable e, boolean isGuinsoo) {
        try {
            ...
            if (e instanceof SQLException) {
                ...
                if (isGuinsoo) {
                    ...
                    error += " <a href=\"https://guinsoodatabase.com/javadoc/" +
                            "org/guinsoo/api/ErrorCode.html#c" + code +
                            "\">(${text.a.help})</a>";
                }
            }
    ...
    private static String linkToSource(String s) {
        try {
            ...
            while (true) {
                int start = s.indexOf("org.guinsoo.", idx);
                if (start < 0) {
                ...
                result.append("<a href=\"https://guinsoodatabase.com/html/source.html?file=");
    ...
    private String test(NetworkConnectionInfo networkConnectionInfo) {
        ...
        boolean isGuinsoo = url.startsWith("jdbc:guinsoo:");
        try {
            ...
            session.put("error", getLoginError(e, isGuinsoo));
            return "login.jsp";
        }
    }
     ...
     * @param isGuinsoo if the current database is a Guinsoo database
     * @return the formatted error message
     */
    private String getLoginError(Exception e, boolean isGuinsoo) {
        if (e instanceof JdbcException && ((JdbcException) e).getErrorCode() == ErrorCode.CLASS_NOT_FOUND_1) {
            return "${text.login.driverNotFound}<br />" + getStackTrace(0, e, isGuinsoo);
        }
        return getStackTrace(0, e, isGuinsoo);
    }
    ...
    private String query() {
        ...
        try {
            ...
            session.put("result", getStackTrace(0, e, session.getContents().isGuinsoo()));
        }
        return "result.jsp";
    }
    ...
    private String editResult() {
        ...
        try {
            ...
            result = "<br />" + getStackTrace(0, e, session.getContents().isGuinsoo());
    ...
    private String getResult(Connection conn, int id, String sql,
        ...
        try {
            ...
            if (sqlUpper.contains("CREATE") ||
                ...
                buff.append("<script type=\"text/javascript\">parent['guinsoomenu'].location='tables.do?jsessionid=")
                        .append(sessionId).append("';</script>");
            }
            Statement stat;
            DbContents contents = session.getContents();
            if (forceEdit || (allowEdit && contents.isGuinsoo())) {
            ...
            return getStackTrace(id, e, session.getContents().isGuinsoo());
        } finally {
    ...
    private String getCommandHistoryString() {
        ...
        for (int i = history.size() - 1; i >= 0; i--) {
            ...
            buff.append("<tr><td><a href=\"getHistory.do?id=").
                append(i).
                append("&jsessionid=${sessionId}\" target=\"guinsooquery\" >").
    ...
    private String getResultSet(String sql, ResultSet rs, boolean metadata,
        ...
        if (edit) {
            buff.append("<form id=\"editing\" name=\"editing\" method=\"post\" " +
                    ...
                    "id=\"mainForm\" target=\"guinsooresult\">" +
        ...
        if (!edit && isUpdatable && allowEdit) {
            buff.append("<br /><br />" +
                    ...
                    "action=\"query.do?jsessionid=${sessionId}\" target=\"guinsooresult\">" +
    ...
    private void unescapeData(String x, ResultSet rs, int columnIndex)
        ...
        if (session.getContents().isGuinsoo()) {
