...
public final class DateTimeFunction extends Function1_2 {
    ...
    private static int extractInterval(Value date, int field) {
        ...
        case MINUTE:
            ...
            break;
        case SECOND:
            v = IntervalUtils.nanosFromInterval(qualifier, negative, leading, remaining) / DateTimeUtils.NANOS_PER_SECOND;
            break;
        case MILLISECOND:
    ...
    static int extractDateTime(SessionLocal session, Value date, int field) {
        ...
        case HOUR:
            return (int) (timeNanos / DateTimeUtils.NANOS_PER_HOUR % 24);
        case MINUTE:
            return (int) (timeNanos / DateTimeUtils.NANOS_PER_MINUTE % 60);
        case SECOND:
            return (int) (timeNanos / DateTimeUtils.NANOS_PER_SECOND % 60);
        case MILLISECOND:
            return (int) (timeNanos / 1_000_000 % 1_000);
        case MICROSECOND:
            return (int) (timeNanos / 1_000 % 1_000_000);
        case NANOSECOND:
            return (int) (timeNanos % DateTimeUtils.NANOS_PER_SECOND);
        case MILLENNIUM:
    ...
    private static Value truncateDate(SessionLocal session, int field, Value value) {
        ...
        case MILLISECOND:
            ...
            break;
        case SECOND:
            timeNanos = timeNanos / DateTimeUtils.NANOS_PER_SECOND * DateTimeUtils.NANOS_PER_SECOND;
            break;
        case MINUTE:
            timeNanos = timeNanos / DateTimeUtils.NANOS_PER_MINUTE * DateTimeUtils.NANOS_PER_MINUTE;
            break;
        case HOUR:
            timeNanos = timeNanos / DateTimeUtils.NANOS_PER_HOUR * DateTimeUtils.NANOS_PER_HOUR;
            break;
        case DAY:
            timeNanos = 0L;
            break;
        ...
        if (session.getMode().getEnum() == Mode.ModeEnum.PostgreSQL && result.getValueType() == Value.DATE) {
    ...
    public static Value dateadd(SessionLocal session, int field, long count, Value v) {
        ...
        case HOUR:
            count *= DateTimeUtils.NANOS_PER_HOUR;
            break;
        case MINUTE:
            count *= DateTimeUtils.NANOS_PER_MINUTE;
            break;
        case SECOND:
        case EPOCH:
            count *= DateTimeUtils.NANOS_PER_SECOND;
            break;
        case MILLISECOND:
            count *= 1_000_000;
            break;
    ...
    private static long datediff(SessionLocal session, int field, Value v1, Value v2) {
        ...
        case HOUR:
            ...
            case MICROSECOND:
                return (absolute2 - absolute1) * (DateTimeUtils.MILLIS_PER_DAY * 1_000) + (timeNanos2 / 1_000 - timeNanos1 / 1_000);
            case MILLISECOND:
                return (absolute2 - absolute1) * DateTimeUtils.MILLIS_PER_DAY + (timeNanos2 / 1_000_000 - timeNanos1 / 1_000_000);
            case SECOND:
            case EPOCH:
                return (absolute2 - absolute1) * 86_400
                        + (timeNanos2 / DateTimeUtils.NANOS_PER_SECOND - timeNanos1 / DateTimeUtils.NANOS_PER_SECOND);
            case MINUTE:
                return (absolute2 - absolute1) * 1_440
                        + (timeNanos2 / DateTimeUtils.NANOS_PER_MINUTE - timeNanos1 / DateTimeUtils.NANOS_PER_MINUTE);
            case HOUR:
                return (absolute2 - absolute1) * 24 + (timeNanos2 / DateTimeUtils.NANOS_PER_HOUR - timeNanos1 / DateTimeUtils.NANOS_PER_HOUR);
            }
            // Fake fall-through
            // $FALL-THROUGH$
        case DAY:
    ...
    public Expression optimize(SessionLocal session) {
        ...
        case DATE_TRUNC: {
            ...
            } else if (session.getMode().getEnum() == Mode.ModeEnum.PostgreSQL && valueType == Value.DATE) {
