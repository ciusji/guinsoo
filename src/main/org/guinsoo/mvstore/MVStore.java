...
import org.guinsoo.compress.CompressDeflate;
import org.guinsoo.compress.CompressLZF;
...
public class MVStore implements AutoCloseable {
     ...
     */
    private volatile long currentStoreVersion = MVMap.INITIAL_VERSION;
                ...
                while (cursor.hasNext() && cursor.next().startsWith(DataUtils.META_CHUNK)) {
                    ...
                    if (!c.isLive()) {
                        ...
                        if (c.unusedAtVersion == 0) {
                            c.unusedAtVersion = MVMap.INITIAL_VERSION;
                        }
                    }
                }
    ...
    public boolean hasUnsavedChanges() {
        ...
        return layout.hasChangesSince(lastStoredVersion) && lastStoredVersion > MVMap.INITIAL_VERSION;
    }
     ...
     * Now it's honored in all cases (although Guinsoo always sets it to zero).
    ...
    long getOldestVersionToKeep() {
        ...
        v = Math.max(v - versionsToKeep, MVMap.INITIAL_VERSION);
        if (fileStore != null) {
            long storeVersion = lastChunkVersion() - 1;
            if (storeVersion != MVMap.INITIAL_VERSION && storeVersion < v) {
                v = storeVersion;
            }
        }
        return v;
    ...
    private long lastChunkVersion() {
        ...
        return chunk == null ? MVMap.INITIAL_VERSION + 1 : chunk.version;
    }
    ...
    public void rollbackTo(long version) {
        ...
        try {
            ...
            if (version == 0) {
                ...
                layout.setInitialRoot(layout.createEmptyLeaf(), MVMap.INITIAL_VERSION);
                meta.setInitialRoot(meta.createEmptyLeaf(), MVMap.INITIAL_VERSION);
                deadChunks.clear();
                removedPages.clear();
                chunks.clear();
                clearCaches();
