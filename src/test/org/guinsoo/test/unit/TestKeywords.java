...
public class TestKeywords extends TestBase {
    ...
    private void testParser() throws Exception {
        try (Connection conn = DriverManager.getConnection("jdbc:guinsoo:mem:keywords")) {
    ...
    private void testInformationSchema() throws Exception {
        try (Connection conn = DriverManager.getConnection("jdbc:guinsoo:mem:")) {
            Statement stat = conn.createStatement();
    ...
    private void testMetaData() throws Exception {
        ...
        set.add("TODAY");
        try (Connection conn = DriverManager.getConnection("jdbc:guinsoo:mem:;OLD_INFORMATION_SCHEMA=TRUE")) {
            assertEquals(setToString(set), conn.getMetaData().getSQLKeywords());
        }
    }
