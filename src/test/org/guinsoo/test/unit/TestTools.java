...
public class TestTools extends TestDb {
    ...
    private void testSimpleResultSet() throws Exception {
        ...
        assertThrows(IllegalStateException.class, () -> r.addColumn(null, 0, 0, 0));
    ...
    private void testJdbcDriverUtils() {
        assertEquals("org.guinsoo.Driver", JdbcUtils.getDriver("jdbc:guinsoo:~/test"));
        ...
        try {
            JdbcUtils.getConnection("org.guinsoo.Driver", "jdbc:guinsoox:test", "sa", "");
    ...
    private void testWrongServer() throws Exception {
        ...
        assertThrows(ErrorCode.CONNECTION_BROKEN_1, () -> getConnection("jdbc:guinsoo:tcp://localhost:9001/test"));
    ...
    private void testNonSSL() throws Exception {
        ...
        try {
            ...
            assertContains(result, "Starts the Guinsoo Console");
            ...
            assertTrue(result.indexOf("Starts the Guinsoo Console") < 0);
            conn = getConnection("jdbc:guinsoo:tcp://localhost:9001/mem:", "sa", "sa");
            conn.close();
    ...
    private void testSSL() throws Exception {
        ...
        try {
            ...
            assertTrue(result.indexOf("Starts the Guinsoo Console") < 0);
            conn = getConnection("jdbc:guinsoo:ssl://localhost:9001/mem:", "sa", "sa");
            conn.close();
            ...
            assertThrows(ErrorCode.CONNECTION_BROKEN_1,
                    () -> getConnection("jdbc:guinsoo:ssl://localhost:9001/mem:", "sa", "sa"));
            ...
            conn = getConnection("jdbc:guinsoo:tcp://localhost:9006/mem:", "sa", "sa");
            conn.close();
            ...
            stop.shutdown();
            assertThrows(ErrorCode.CONNECTION_BROKEN_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:9006/mem:", "sa", "sa"));
        } finally {
            shutdownServers();
        }
    }
    ...
    private void testConvertTraceFile() throws Exception {
        ...
        Driver.load();
        String url = "jdbc:guinsoo:" + getBaseDir() + "/toolsConvertTraceFile";
        url = getURL(url, true);
        ...
        deleteDb("toolsConvertTraceFile");
        FileUtils.delete(getBaseDir() + "/toolsConvertTraceFile.guinsoo.sql");
    ...
    private void testRemove() throws SQLException {
        ...
        Driver.load();
        String url = "jdbc:guinsoo:" + getBaseDir() + "/toolsRemove";
        ...
        conn.close();
        deleteDb("toolsRemove");
        FileUtils.delete(getBaseDir() + "/toolsRemove.guinsoo.sql");
    }
    ...
    private void testRecover() throws SQLException {
        ...
        conn.close();
        deleteDb("toolsRecover");
        FileUtils.delete(getBaseDir() + "/toolsRecover.guinsoo.sql");
    ...
    private void testScriptRunscriptLob() throws Exception {
        String url = getURL("jdbc:guinsoo:" + getBaseDir() +
        ...
        for (int i = 0; i < 2; i++) {
            ...
            RunScript.main("-url", url, "-user", user, "-password", password,
                    "-script", fileName);
            conn = getConnection("jdbc:guinsoo:" + getBaseDir() +
    ...
    private void testScriptRunscript() throws Exception {
        String url = getURL("jdbc:guinsoo:" + getBaseDir() + "/testScriptRunscript",
                true);
        String user = "sa", password = "abc";
        String fileName = getBaseDir() + "/b2.sql";
        ...
        conn = getConnection(
                "jdbc:guinsoo:" + getBaseDir() + "/testScriptRunscript", "sa", "abc");
    ...
    private void testBackupRestore() throws SQLException {
        Driver.load();
        String url = "jdbc:guinsoo:" + getBaseDir() + "/testBackupRestore";
        String user = "sa", password = "abc";
        ...
        Restore.main("-file", fileName, "-dir", getBaseDir(), "-db",
                "testBackupRestore", "-quiet");
        conn = getConnection("jdbc:guinsoo:" + getBaseDir() + "/testBackupRestore",
                "sa", "abc");
        ResultSet rs = conn.createStatement()
                .executeQuery("SELECT * FROM TEST");
        assertTrue(rs.next());
    ...
    private void testChangeFileEncryption(boolean split) throws SQLException {
        ...
        String url = "jdbc:guinsoo:" + dir + "/testChangeFileEncryption;CIPHER=AES";
    ...
    private void testChangeFileEncryptionWithWrongPassword() throws SQLException {
        ...
        // file handle open
        assertThrows(SQLException.class, () -> ChangeFileEncryption.execute(dir, "testChangeFileEncryption", "AES",
                "wrong".toCharArray(), "def".toCharArray(), true));
    ...
    private void testServer() throws SQLException {
        ...
        try {
            ...
            conn = getConnection("jdbc:guinsoo:tcp://localhost:" + port + "/test", "sa", "");
            conn.close();
            // must not be able to use a different base dir
            assertThrows(ErrorCode.IO_EXCEPTION_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:" + port + "/../test", "sa", ""));
            assertThrows(ErrorCode.IO_EXCEPTION_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:" + port + "/../test2/test", "sa", ""));
            ...
            assertThrows(ErrorCode.REMOTE_DATABASE_NOT_FOUND_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:" + prt + "/test2", "sa", ""));
            assertThrows(ErrorCode.REMOTE_DATABASE_NOT_FOUND_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:" + prt + "/test2;ifexists=false", "sa", ""));
            conn = getConnection("jdbc:guinsoo:tcp://localhost:" + prt + "/test", "sa", "");
            conn.close();
            assertThrows(ErrorCode.WRONG_USER_OR_PASSWORD,
            ...
            assertThrows(ErrorCode.CONNECTION_BROKEN_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:" + prt + "/test", "sa", ""));
            ...
            conn = getConnection("jdbc:guinsoo:tcp://localhost:" + p + "/split:testSplit", "sa", "");
            conn.close();

            assertThrows(ErrorCode.IO_EXCEPTION_1,
                    () -> getConnection("jdbc:guinsoo:tcp://localhost:" + p + "/../test", "sa", ""));
