...
public class TestClassLoaderLeak extends TestBase {
    ...
    public void test() throws Exception {
        ...
        DriverManager.registerDriver((Driver)
                Class.forName("org.guinsoo.Driver").getDeclaredConstructor().newInstance());
        DriverManager.registerDriver((Driver)
                Class.forName("org.guinsoo.upgrade.v1_1.Driver").getDeclaredConstructor().newInstance());
    }

    private static WeakReference<ClassLoader> createClassLoader() throws Exception {
        ClassLoader cl = new TestClassLoader();
        Class<?> guinsooConnectionTestClass = Class.forName(CLASS_NAME, true, cl);
        Method testMethod = guinsooConnectionTestClass.getDeclaredMethod("runTest");
    ...
    static void runTest() throws Exception {
        ...
        Driver d1 = DriverManager.getDriver("jdbc:guinsoo:mem:test");
        Driver d2 = DriverManager.getDriver("jdbc:guinsoov1_1:mem:test");
        Connection connection;
        connection = DriverManager.getConnection("jdbc:guinsoo:mem:test");
    ...
    private static class TestClassLoader extends URLClassLoader {
        ...
        // allows delegation of Guinsoo to the AppClassLoader
        @Override
        public synchronized Class<?> loadClass(String name, boolean resolve)
                throws ClassNotFoundException {
            if (!name.contains(CLASS_NAME) && !name.startsWith("org.guinsoo.")) {
