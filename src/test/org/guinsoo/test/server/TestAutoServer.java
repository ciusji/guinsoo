...
public class TestAutoServer extends TestDb {
    ...
    private void testUnsupportedCombinations() {
        String[] urls = {
                "jdbc:guinsoo:" + getTestName() + ";file_lock=no;auto_server=true",
                "jdbc:guinsoo:" + getTestName() + ";file_lock=serialized;auto_server=true",
                "jdbc:guinsoo:" + getTestName() + ";access_mode_data=r;auto_server=true",
                "jdbc:guinsoo:mem:" + getTestName() + ";auto_server=true"
        };
        for (String url : urls) {
        ...
        try (Connection connServer = getConnection(url + ";OPEN_NEW=TRUE", user, password)) {
            ...
            for (; i > 0; i--) {
                ...
                if (server != null) {
                    ...
                    u2 = "jdbc:guinsoo:tcp://" + server + "/" + key + u2;
    ...
    private void testSocketReadTimeout(boolean port) throws Exception {
        ...
        try {
            ...
            if (server != null) {
                ...
                u2 = "jdbc:guinsoo:tcp://" + server + "/" + key + u2 + ";NETWORK_TIMEOUT=100";
                Connection conn = DriverManager.getConnection(u2, user, password);
                Statement stat = conn.createStatement();
