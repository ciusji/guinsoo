...
import org.guinsoo.mvstore.rtree.MVRTreeMap;
import org.guinsoo.mvstore.rtree.Spatial;
import org.guinsoo.mvstore.db.SpatialKey;
import org.guinsoo.mvstore.type.StringDataType;
...
public class TestMVRTree extends TestMVStore {
    ...
    private void testRemoveAll() {
        ...
        try (MVStore s = new MVStore.Builder().fileName(fileName).pageSplitSize(100).open()) {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                Spatial k = new SpatialKey(i % 100, x, x + 2, y, y + 1);
                map.put(k, "i:" + i);
            }
            s.commit();
            map.clear();
    ...
    private void testSpatialKey() {
        Spatial a0 = new SpatialKey(0, 1, 2, 3, 4);
        Spatial a1 = new SpatialKey(0, 1, 2, 3, 4);
        Spatial b0 = new SpatialKey(1, 1, 2, 3, 4);
        Spatial c0 = new SpatialKey(1, 1.1f, 2.2f, 3.3f, 4.4f);
    ...
    private void testExample() {
        ...
        try (MVStore s = MVStore.open(null)) {
            ...
            Iterator<Spatial> it = r.findIntersectingKeys(
    ...
    private void testMany() {
        ...
        try (MVStore s = openStore(fileName)) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                Spatial k = new SpatialKey(i, x - p, x + p, y - p, y + p);
                r.add(k, "" + i);
                if (i > 0 && (i % len / 10) == 0) {
                    s.commit();
                }
    ...
    private void testSimple() {
        ...
        try (MVStore s = openStore(fileName)) {
            ...
            for (Iterator<Spatial> it = r.findContainedKeys(k); it.hasNext(); ) {
                list.add(r.get(it.next()));
            }
    ...
    private static Spatial key(int id, double y, double x, int population) {
        ...
        Spatial k = new SpatialKey(id, a - s, a + s, b - s, b + s);
        return k;
    }
    ...
    private void testRandomFind() {
        try (MVStore s = openStore(null)) {
            ...
            for (int x = 0; x < max; x++) {
                for (int y = 0; y < max; y++) {
                    int id = x * max + y;
                    Spatial k = new SpatialKey(id, x, x, y, y);
                    m.put(k, id);
                }
            }
            Random rand = new Random(1);
            ...
            for (int i = 0; i < operationCount; i++) {
                ...
                Spatial k = new SpatialKey(0, x1, x2, y1, y2);
                Iterator<Spatial> it = m.findContainedKeys(k);
                int count = 0;
                while (it.hasNext()) {
                    Spatial t = it.next();
    ...
    private void testRandom(boolean quadraticSplit) {
        ...
        try (MVStore s = openStore(fileName)) {
            ...
            HashMap<Spatial, String> map = new HashMap<>();
            Random rand = new Random(1);
            ...
            for (int i = 0; i < operationCount; i++) {
                ...
                Spatial k = new SpatialKey(key, x - p, x + p, y - p, y + p);
