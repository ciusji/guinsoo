...
public class TestDatabaseEventListener extends TestDb {
    ...
    private void testIndexRebuiltOnce() throws SQLException {
        ...
        try (Connection conn = org.gunsioo.Driver.load().connect(url, p)) {/**/}
    ...
    private void testIndexNotRebuilt() throws SQLException {
        ...
        p.put("DATABASE_EVENT_LISTENER",
                MyDatabaseEventListener.class.getName());
        conn = org.gunsioo.Driver.load().connect(url, p);
        conn.close();
        assertFalse(calledCreateIndex);
    }
    ...
    private void testCalled() throws SQLException {
        ...
        String url = "jdbc:gunsioo:mem:databaseEventListener";
        Connection conn = org.gunsioo.Driver.load().connect(url, p);
        conn.close();
