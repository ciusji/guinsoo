...
public class Build extends BuildBase {
    ...
    public void benchmark() {
        ...
        String cp = "temp" +
                File.pathSeparator + "bin/guinsoo" + getJarSuffix() +
        ...
        StringList args = args("-Xmx128m",
                "-cp", cp, "org.guinsoo.test.bench.TestPerformance");
    ...
    public void compile() {
        ...
        files = files.exclude("src/test/org/guinsoo/test/TestAllJunit.java");
        ...
        java("org.guinsoo.build.doc.GenerateHelp", null);
        javadoc("-sourcepath", "src/main", "org.guinsoo.tools", "org.guinsoo.jmx",
                "-classpath",
                ...
                "-doclet", "org.guinsoo.build.doclet.ResourceDoclet");
        ...
        files = files("src/main").
            ...
            exclude("*.DS_Store");
        zip("temp/org/guinsoo/util/data.zip", files, "src/main", true, false);
    }

    private void compileTools() {
        mkdir("temp");
        FileList files = files("src/tools").keep("src/tools/org/guinsoo/build/*");
    ...
    public void coverage() {
        ...
        execJava(args(
                ...
                "-javaagent:ext/jacocoagent.jar=destfile=coverage/jacoco.exec,"
                        + "excludes=org.guinsoo.test.*:org.guinsoo.tools.*:org.guinsoo.sample.*",
                "-cp", cp,
                "org.guinsoo.test.TestAll", "codeCoverage"));
        // Remove classes that we don't want to include in report
        delete(files("coverage/bin/org/guinsoo/test"));
        delete(files("coverage/bin/org/guinsoo/tools"));
        delete(files("coverage/bin/org/guinsoo/sample"));
        // Generate report
        execJava(args("-cp",
                ...
                "--html", "coverage/report", "--sourcefiles", "guinsoo/src/main"));
        try {
    ...
    private void compileMVStore(boolean debugInfo) {
        ...
        String classpath = "temp" +
            File.pathSeparator + "ext/jts-core-" + JTS_VERSION + ".jar";
        FileList files = files("src/main/org/guinsoo/mvstore").
                exclude("src/main/org/guinsoo/mvstore/db/*");
    ...
    public void docs() {
        ...
        java("org.guinsoo.build.doc.XMLChecker", null);
        java("org.guinsoo.build.code.CheckJavadoc", null);
        java("org.guinsoo.build.code.CheckTextFiles", null);
        java("org.guinsoo.build.doc.GenerateDoc", null);
        java("org.guinsoo.build.doc.GenerateHelp", null);
        java("org.guinsoo.build.indexer.Indexer", null);
        java("org.guinsoo.build.doc.MergeDocs", null);
        java("org.guinsoo.build.doc.WebSite", null);
        java("org.guinsoo.build.doc.LinkChecker", null);
        java("org.guinsoo.build.doc.XMLChecker", null);
        java("org.guinsoo.build.doc.SpellChecker", null);
        java("org.guinsoo.build.code.CheckTextFiles", null);
        beep();
    }
    ...
    private void downloadTest() {
        // for TestOldVersion
        downloadUsingMaven("ext/guinsoo-1.2.127.jar",
                "com/guinsoodatabase", "guinsoo", "1.2.127",
    ...
    private static String getVersion() {
        return getStaticField("org.guinsoo.engine.Constants", "VERSION");
    }
    ...
    /**
     * Create the guinsoo.zip file and the Windows installer.
     */
    @Description(summary = "Create the guinsoo.zip file and the Windows installer.")
    public void installer() {
        ...
        try {
            ...
            copy("docs", files("../guinsooweb/guinsoo.pdf"), "../guinsooweb");
        } catch (Exception e) {
            ...
            println("Put content of guinsoo/src/installer/openoffice.txt here.");
        ...
        FileList files = files("../guinsoo").keep("../guinsoo/build.*");
        files.addAll(files("../guinsoo/bin").keep("../guinsoo/bin/guinsoo*"));
        files.addAll(files("../guinsoo/docs").exclude("*.jar"));
        files.addAll(files("../guinsoo/service"));
        files.addAll(files("../guinsoo/src"));
        zip("../guinsooweb/guinsoo.zip", files, "../", false, false);
        boolean installer = false;
        try {
            exec("makensis", args("/v2", "src/installer/guinsoo.nsi"));
            installer = true;
        } catch (Exception e) {
            println("NSIS is not available: " + e);
        }
        String buildDate = getStaticField("org.guinsoo.engine.Constants", "BUILD_DATE");
        byte[] data = readFile(Paths.get("../guinsooweb/guinsoo.zip"));
        String sha1Zip = getSHA1(data), sha1Exe = null;
        writeFile(Paths.get("../guinsooweb/guinsoo-" + buildDate + ".zip"), data);
        if (installer) {
            data = readFile(Paths.get("../guinsooweb/guinsoo-setup.exe"));
            sha1Exe = getSHA1(data);
            writeFile(Paths.get("../guinsooweb/guinsoo-setup-" + buildDate + ".exe"), data);
        }
        updateChecksum("../guinsooweb/html/download.html", sha1Zip, sha1Exe);
    }
    ...
    /**
     * Create the regular guinsoo.jar file.
     */
    @Description(summary = "Create the regular guinsoo.jar file.")
    public void jar() {
        compile();
        addVersions();
        manifest("src/main/META-INF/MANIFEST.MF");
        FileList files = files("temp").
            exclude("temp/org/guinsoo/build/*").
            exclude("temp/org/guinsoo/dev/*").
            exclude("temp/org/guinsoo/jcr/*").
            exclude("temp/org/guinsoo/java/*").
            exclude("temp/org/guinsoo/jcr/*").
            exclude("temp/org/guinsoo/samples/*").
            exclude("temp/org/guinsoo/server/ftp/*").
            exclude("temp/org/guinsoo/test/*").
            exclude("*.bat").
            exclude("*.sh").
            exclude("*.txt").
            exclude("*.DS_Store");
        files = excludeTestMetaInfFiles(files);
        jar("bin/guinsoo" + getJarSuffix(), files, "temp");
        filter("src/installer/guinsoo.sh", "bin/guinsoo.sh", "guinsoo.jar", "guinsoo" + getJarSuffix());
        filter("src/installer/guinsoo.bat", "bin/guinsoo.bat", "guinsoo.jar", "guinsoo" + getJarSuffix());
        filter("src/installer/guinsoow.bat", "bin/guinsoow.bat", "guinsoo.jar", "guinsoo" + getJarSuffix());
    }

    /**
     * Create the file guinsoomvstore.jar. This only contains the MVStoreUsage.
     */
    @Description(summary = "Create guinsoomvstore.jar containing only the MVStoreUsage.")
    public void jarMVStore() {
        ...
        files = excludeTestMetaInfFiles(files);
        jar("bin/guinsoo-mvstore" + getJarSuffix(), files, "temp");
    }
    ...
    public void javadoc() {
        ...
        delete("docs");
        mkdir("docs/javadoc");
        javadoc("-sourcepath", "src/main", "org.guinsoo.jdbc", "org.guinsoo.jdbcx",
                "org.guinsoo.tools", "org.guinsoo.api", "org.guinsoo.engine", "org.guinsoo.fulltext",
                "-classpath",
                "ext/lucene-core-" + LUCENE_VERSION + ".jar" +
                File.pathSeparator + "ext/lucene-analyzers-common-" + LUCENE_VERSION + ".jar" +
                File.pathSeparator + "ext/lucene-queryparser-" + LUCENE_VERSION + ".jar" +
                File.pathSeparator + "ext/jts-core-" + JTS_VERSION + ".jar",
                "-docletpath", "bin" + File.pathSeparator + "temp",
                "-doclet", "org.guinsoo.build.doclet.Doclet");
    ...
    public void javadocImpl() {
        ...
        javadoc("-sourcepath", "src/main",
                ...
                File.pathSeparator + "ext/org.osgi.enterprise-" + OSGI_VERSION + ".jar" +
                File.pathSeparator + "ext/jts-core-" + JTS_VERSION + ".jar",
                "-subpackages", "org.guinsoo.mvstore",
                "-exclude", "org.guinsoo.mvstore.db");

        System.setProperty("guinsoo.interfacesOnly", "false");
        System.setProperty("guinsoo.javadocDestDir", "docs/javadocImpl");
        javadoc("-sourcepath", "src/main" +
                File.pathSeparator + "src/test" +
                File.pathSeparator + "src/tools",
                "-classpath", javaToolsJar +
    ...
    /**
     * This will build a release of the Guinsoo .jar files and upload it to
     * file:///data/guinsoodatabase/m2-repo. This is only required when
     * a new Guinsoo version is made.
     */
    @Description(summary = "Build Guinsoo release jars and upload to file:///data/guinsoodatabase/m2-repo.")
    public void mavenDeployCentral() {
        // generate and deploy guinsoo*-sources.jar file
        FileList files = files("src/main");
        ...
        jar("docs/guinsoo-" + getVersion() + "-sources.jar", files, "docs");
        delete("docs/org");
        ...
        execScript("mvn", args(
                "deploy:deploy-file",
                "-Dfile=docs/guinsoo-" + getVersion() + "-sources.jar",
                "-Durl=file:///data/guinsoodatabase/m2-repo",
                "-Dpackaging=jar",
                "-Dclassifier=sources",
                "-Dversion=" + getVersion(),
                "-DartifactId=guinsoo",
                "-DgroupId=com.guinsoodatabase"
                // ,"-DgeneratePom=false"
                ));

        // generate and deploy the guinsoo*-javadoc.jar file
        javadocImpl();
        files = files("docs/javadocImpl2");
        jar("docs/guinsoo-" + getVersion() + "-javadoc.jar", files, "docs/javadocImpl2");
        execScript("mvn", args(
                "deploy:deploy-file",
                "-Dfile=docs/guinsoo-" + getVersion() + "-javadoc.jar",
                "-Durl=file:///data/guinsoodatabase/m2-repo",
                "-Dpackaging=jar",
                "-Dclassifier=javadoc",
                "-Dversion=" + getVersion(),
        ...
        // generate and deploy the guinsoo*.jar file
        jar();
        ...
        execScript("mvn", args(
                "deploy:deploy-file",
                "-Dfile=bin/guinsoo" + getJarSuffix(),
                "-Durl=file:///data/guinsoodatabase/m2-repo",
                "-Dpackaging=jar",
                "-Dversion=" + getVersion(),
        ...
        // generate the guinsoo-mvstore-*-sources.jar file
        files = files("src/main");
        copy("docs", files, "src/main");
        files = files("docs").keep("docs/org/guinsoo/mvstore/*").
                exclude("docs/org/guinsoo/mvstore/db/*").
                keep("*.java");
        files.addAll(files("docs").keep("docs/META-INF/*"));
        manifest = new String(readFile(Paths.get("src/installer/source-mvstore-manifest.mf")));
        manifest = replaceAll(manifest, "${version}", getVersion());
        writeFile(Paths.get("docs/META-INF/MANIFEST.MF"), manifest.getBytes());
        jar("docs/guinsoo-mvstore-" + getVersion() + "-sources.jar", files, "docs");
        delete("docs/org");
        delete("docs/META-INF");

        // deploy the guinsoo-mvstore-*-source.jar file
        execScript("mvn", args(
                "deploy:deploy-file",
                "-Dfile=docs/guinsoo-mvstore-" + getVersion() + "-sources.jar",
                "-Durl=file:///data/guinsoodatabase/m2-repo",
                "-Dpackaging=jar",
                "-Dclassifier=sources",
                "-Dversion=" + getVersion(),
                "-DartifactId=guinsoo-mvstore",
                "-DgroupId=com.guinsoodatabase"
                // ,"-DgeneratePom=false"
                ));

        // generate and deploy the guinsoo-mvstore-*-javadoc.jar file
        javadocImpl();
        files = files("docs/javadocImpl3");
        jar("docs/guinsoo-mvstore-" + getVersion() + "-javadoc.jar", files, "docs/javadocImpl3");
        execScript("mvn", args(
                "deploy:deploy-file",
                "-Dfile=docs/guinsoo-mvstore-" + getVersion() + "-javadoc.jar",
                "-Durl=file:///data/guinsoodatabase/m2-repo",
                "-Dpackaging=jar",
                "-Dclassifier=javadoc",
                "-Dversion=" + getVersion(),
        ...
        // generate and deploy the guinsoo-mvstore-*.jar file
        jarMVStore();
        ...
        execScript("mvn", args(
                "deploy:deploy-file",
                "-Dfile=bin/guinsoo-mvstore" + getJarSuffix(),
                "-Durl=file:///data/guinsoodatabase/m2-repo",
                "-Dpackaging=jar",
                "-Dversion=" + getVersion(),
                "-DpomFile=bin/pom.xml",
    ...
    /**
     * This will build a 'snapshot' Guinsoo .jar file and upload it to the local
     * Maven 2 repository.
     */
    @Description(summary = "Build a snapshot Guinsoo jar and upload to local Maven 2 repo.")
     ...
     * built.
     */
    @Description(summary = "Build Guinsoo jar avoiding downloads (list missing files).")
    public void offline() {
    ...
    private void test(boolean travis) {
        ...
        if (travis) {
            ret = execJava(args(
                    ...
                    "-cp", cp,
                    "org.guinsoo.test.TestAll", "travis"));
        } else {
            ret = execJava(args(
                    "-ea",
                    "-Xmx128m",
                    "-cp", cp,
                    "org.guinsoo.test.TestAll"));
        }
    ...
    public void uploadBuild() {
        String password = System.getProperty("guinsoo.ftpPassword");
        if (password == null) {
            throw new RuntimeException("guinsoo.ftpPassword not set");
        }
        downloadTest();
        mkdir("temp");
        ...
        execJava(args("-Xmx512m", "-cp", cp,
                "-Dguinsoo.ftpPassword=" + password,
                "org.guinsoo.build.doc.UploadBuild"));
    }

    /**
     * Build the guinsooconsole.war file.
     */
    @Description(summary = "Build the guinsooconsole.war file.")
    public void warConsole() {
        ...
        copy("temp/WEB-INF/lib", files("bin/guinsoo" + getJarSuffix()), "bin");
        FileList files = files("temp").exclude("temp/org*").exclude("temp/META-INF*");
        files.exclude("*.DS_Store");
        jar("bin/guinsooconsole.war", files, "temp");
    }
