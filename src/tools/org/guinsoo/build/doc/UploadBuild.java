...
/**
 * Upload the code coverage result to the Guinsoo web site.
 */
public class UploadBuild {
    ...
    public static void main(String... args) throws Exception {
        System.exit(0);
        System.setProperty("guinsoo.socketConnectTimeout", "30000");
        String password = System.getProperty("guinsoo.ftpPassword");
        if (password == null) {
            return;
        }
        FtpClient ftp = FtpClient.open("guinsoodatabase.com");
        ftp.login("guinsoodatabase", password);
        ...
        String sql = "insert into item(title, issued, desc) values('Build " +
            ...
            "', '" + ts +
            "', '<a href=\"https://guinsoodatabase.com/" +
            "html/testOutput.html\">Output</a>" +
            " - <a href=\"https://guinsoodatabase.com/" +
            "coverage/overview.html\">Coverage</a>" +
            " - <a href=\"https://guinsoodatabase.com/" +
            "automated/guinsoo-latest.jar\">Jar</a>');\n";
        buildSql += sql;
        Connection conn;
        try {
            Class.forName("org.guinsoo.Driver");
            conn = DriverManager.getConnection("jdbc:guinsoo:mem:");
        } catch (Exception e) {
            Class.forName("org.guinsoo.upgrade.v1_1.Driver");
            conn = DriverManager.getConnection("jdbc:guinsoov1_1:mem:");
        }
        conn.createStatement().execute(buildSql);
        String newsfeed = new String(Files.readAllBytes(Paths.get("src/tools/org/guinsoo/build/doc/buildNewsfeed.sql")),
        ...
        String jarFileName = "bin/guinsoo-" + Constants.VERSION + ".jar";
        if (FileUtils.exists(jarFileName)) {
            ftp.store("/httpdocs/automated/guinsoo-latest.jar",
        ...
        boolean mavenSnapshot = Files.exists(Paths.get(mavenRepoDir +
                "com/guinsoodatabase/guinsoo/1.0-SNAPSHOT/guinsoo-1.0-SNAPSHOT.jar"));
        if (mavenSnapshot) {
            ...
            if (!ftp.exists("/httpdocs/m2-repo/com", "guinsoodatabase")) {
                ftp.makeDirectory("/httpdocs/m2-repo/com/guinsoodatabase");
            }
            if (!ftp.exists("/httpdocs/m2-repo/com/guinsoodatabase", "guinsoo")) {
                ftp.makeDirectory("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo");
            }
            if (!ftp.exists("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo", "1.0-SNAPSHOT")) {
                ftp.makeDirectory("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo/1.0-SNAPSHOT");
            }
            if (!ftp.exists("/httpdocs/m2-repo/com/guinsoodatabase", "guinsoo-mvstore")) {
                ftp.makeDirectory("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo-mvstore");
            }
            if (!ftp.exists("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo-mvstore", "1.0-SNAPSHOT")) {
                ftp.makeDirectory("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo-mvstore/1.0-SNAPSHOT");
            }
            ftp.store("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo" +
                    "/1.0-SNAPSHOT/guinsoo-1.0-SNAPSHOT.pom",
                    Files.newInputStream(Paths.get(mavenRepoDir +
                            "com/guinsoodatabase/guinsoo/1.0-SNAPSHOT/guinsoo-1.0-SNAPSHOT.pom")));
            ftp.store("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo" +
                            "/1.0-SNAPSHOT/guinsoo-1.0-SNAPSHOT.jar",
                    Files.newInputStream(Paths.get(mavenRepoDir +
                            "com/guinsoodatabase/guinsoo/1.0-SNAPSHOT/guinsoo-1.0-SNAPSHOT.jar")));
            ftp.store("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo-mvstore" +
                            "/1.0-SNAPSHOT/guinsoo-mvstore-1.0-SNAPSHOT.pom",
                    Files.newInputStream(Paths.get(mavenRepoDir +
                            "com/guinsoodatabase/guinsoo-mvstore/1.0-SNAPSHOT/guinsoo-mvstore-1.0-SNAPSHOT.pom")));
            ftp.store("/httpdocs/m2-repo/com/guinsoodatabase/guinsoo-mvstore" +
                            "/1.0-SNAPSHOT/guinsoo-mvstore-1.0-SNAPSHOT.jar",
                    Files.newInputStream(Paths.get(mavenRepoDir +
                            "com/guinsoodatabase/guinsoo-mvstore/1.0-SNAPSHOT/guinsoo-mvstore-1.0-SNAPSHOT.jar")));
        }
        ftp.close();
    }
